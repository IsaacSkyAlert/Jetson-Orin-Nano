...
Con límites de ejes Y ajustados dinámicamente y identificador de estación
"""
...
# Factor de división para RVM_PEAK (nueva configuración)
...
class AnyShakeRealTimePlotter:
    def __init__(self, host='localhost', port=30000, display_seconds=30):
        ...
        # Configuración de la visualización - 5 subgráficas
        self.fig, self.axes = plt.subplots(5, 1, figsize=(14, 12))
        self.ax1, self.ax2, self.ax3, self.ax4, self.ax5 = self.axes
        
        self.fig.suptitle(f'AnyShake - {SAMPLING_RATE} Hz - RVM + STA/LTA + Anomaly Detection - Station {STATION_ID}', fontsize=12)
        ...
        # Líneas de gráfico
        self.line_enz, = self.ax1.plot([], [], 'b-', linewidth=0.5)
        self.line_ene, = self.ax2.plot([], [], 'r-', linewidth=0.5)
        self.line_enn, = self.ax3.plot([], [], 'g-', linewidth=0.5)
        self.line_rvm, = self.ax4.plot([], [], 'm-', linewidth=0.8)
        self.line_sta_lta, = self.ax5.plot([], [], 'k-', linewidth=1.0)
        
        # Configurar ejes
        components_config = [
            (self.ax1, 'Componente Vertical (ENZ)', 'b'),
            (self.ax2, 'Componente Este-Oeste (ENE)', 'r'),
            (self.ax3, 'Componente Norte-Sur (ENN)', 'g'),
            (self.ax4, 'Energía Vectorial (RVM)', 'm'),
            (self.ax5, f'STA/LTA Ratio (Thr_ON={THR_ON}, Thr_OFF={THR_OFF})', 'k')
        ]
        ...
        for ax, title, color in components_config:
            if ax == self.ax5:
                ...
                ax.set_ylabel('Aceleración (Gals)')
            ax.set_title(title)
            ax.grid(True, alpha=0.3)
            ax.axhline(y=0, color='k', linestyle='-', alpha=0.3)
        
        self.ax5.set_xlabel('Tiempo (segundos)')
        
        # Umbrales de detección
        self.ax5.axhline(y=THR_ON, color='r', linestyle='--', alpha=0.7, label=f'Thr_ON={THR_ON}')
        self.ax5.axhline(y=THR_OFF, color='g', linestyle='--', alpha=0.7, label=f'Thr_OFF={THR_OFF}')
        self.ax5.legend(fontsize=8)
        
        # Texto para mostrar estado de anomalía
    ...
    def start(self):
        ...
        print(f"Sistema de monitoreo sísmico iniciado - Estación {STATION_ID}")
    ...
    def stop(self):
        ...
        # Reporte final
        status = self.anomaly_detector.get_status()
        print(f"\n--- REPORTE FINAL - Estación {STATION_ID} ---")
